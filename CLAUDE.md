...
- **`onIconClick`** — makes the icon an interactive button. Clickable icons render inside a bordered `ActionIcon` so the affordance is visible at rest; never rely on colour to imply clickability.
